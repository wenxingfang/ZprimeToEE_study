...
#define LOGVERB(x) if(false) std::cout <<x<<" "
#define LOGWARN(x) if(false) std::cout <<x<<" "
#define LOGERR(x) std::cout <<x<<" "
#define LOGDRESSED(x) if(false) std::cout <<x<<" "

namespace edm {
  class FileInPath {
  public:
    FileInPath(std::string str):str_(std::move(str)){}
    FileInPath(const TString& str):str_(str.Data()){}
    const std::string& fullPath()const{return str_;}
  private:
    std::string str_;
  };
}
  ...
  smearingType_(ECALELF)
{
  ...
  if(correctionFileName.size() > 0) { 
    ...
    if(scales.empty()) {
      std::cout <<"exception" << "[ERROR] scale correction map empty";
      exit(0);
    }
  }
  ...
  if(correctionFileName.size() > 0) { 
    ...
    if(smearings.empty()) {
      std::cout <<"exception" << "[ERROR] smearing correction map empty";
      exit(0);
    }
  }
...
void EnergyScaleCorrection_class::ReadFromFile(TString filename)
{
  ...
  if(!f_in.good()) {
    std::cout <<"exception" << "[ERROR] file " << filename << " not readable.";
    exit(0);
  }
