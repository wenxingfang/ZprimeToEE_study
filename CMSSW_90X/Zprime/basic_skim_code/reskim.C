...
#include "/user/wenxing/ST_TW_channel/CMSSW_9_4_0/src/Zprime/basic_skim_code/L1Bit2017.h"
...
//// clear vector //////////////////////
      ...
      L1_bit = L1Bit(int(ev_run), TString("L1_DoubleEG_25_14"));
...
//#############   For electrons ##########################
      ...
      for(unsigned int iEl=0 ; iEl<gsf_n ; ++iEl){
        ...
        if(fabs(sc_eta)<1.4442)Et = Et*0.997;
        else if(fabs(sc_eta)>1.566 && fabs(sc_eta)<2.5) Et = Et*1.004;
        }
        if(do_EnergySmear==true){
        if(fabs(sc_eta)<1.4442)Et = Et*rand.Gaus(1,0.00728) ;
        else if(fabs(sc_eta)>1.566 && fabs(sc_eta)<2.5) Et = Et*rand.Gaus(1,0.021002) ;
        }
        if(do_officical_EnergyScaleAndSmear){ sc_Et_corrected=sc_Et*egamma::eCorr(ev_run, Et, sc_eta, gsf_r9->at(iEl), 12, !isData);
                                              Et=Et*egamma::eCorr(ev_run, Et, sc_eta, gsf_r9->at(iEl), 12, !isData);
                                             }
...
//      if(ev_event==105 || ev_event==19812 || ev_event==20085) std::cout<<"ev:"<<ev_event<<",pass="<<for_Zprime<<"n 74:"<<heeps.size()<<"n 80:"<<heeps_noEcalDriven.size()<<"n b up:"<<heeps_Barrel_EScale_up.size()<<"n b down:"<<heeps_Barrel_EScale_down.size()<<"n e up:"<<heeps_Endcap_EScale_up.size()<<"n e down:"<<heeps_Endcap_EScale_down.size()<<std::endl;
      if( for_tw || for_Zprime ){
      
//      L1_pass_final_out     = isData ? int(L1_pass_final->at(L1_bit)) : 0 ;
