...
//   const int NAltCut     =  4 ; nHistos *= NAltCut     ;
   const int NAltCut     =  14 ; nHistos *= NAltCut     ;
//   const int NJson       =  5 ; nHistos *= NJson       ;
   const int NJson       =  3 ; nHistos *= NJson       ;
   const int NPUW        =  2 ; nHistos *= NPUW        ;
   
   TFile* fBase = new TFile("/user/wenxing/HEEP/CMSSW_7_4_10_patch2/src/HEEPScaleFactorStudy/HEEPScaleFactorStudy_90X/hBase.root","READ") ;
   
   TFile* file_out = new TFile(Form("/user/wenxing/HEEP/CMSSW_7_4_10_patch2/src/HEEPScaleFactorStudy/HEEPScaleFactorStudy_90X/ntuples/out/simple/%s_slices.root",sampleName.Data()),"RECREATE") ;
...
//   TString tagChargeNames[NTagCharges] = {"tm","tp","ta"} ;
   ...
   TString    altCutNames[NAltCut    ] = {"nominal","N_1_EcalDriven","N_1_DEtaIn","N_1_DPhiIn","N_1_Shower","N_1_TrkIso","N_1_EMHD1Iso","N_1_MissHit","N_1_Dxy","N_1_HoE","N_2_Isolation","N_3_Trk","N_2_DetaIn_Dxy","noEcalDriven"} ;
...
//      if(pass_HLT_Ele32_require!=1) continue;//the tag and probe need pass the trigger requirement
//      if(pass_HLT_Ele32_require_v1!=1) continue;//the tag and probe need pass the trigger requirement
      if(pass_HLT_Ele32_require_v2!=1) continue;//the tag and probe need pass the trigger requirement
//      if(pass_HLT_Ele35_require!=1) continue;//the tag and probe need pass the trigger requirement
...
case 13:{ pass_or_fail =(p_ID_noEcalDriven) ? 1 : 2 ; fill_1HEEP= true ; break;}
default:{std::cout<<"Error, out of range"<<std::endl;}
            }
//std::cout << "after case:"<<iA<< std::endl ;
