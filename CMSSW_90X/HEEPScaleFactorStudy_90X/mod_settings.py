...
lumi_by_campaign = {}
lumi_by_campaign['golden2015D'     ] = 4889 
lumi_by_campaign['silver'          ] = 4889 
lumi_by_campaign['silverUp'        ] = 4889 
lumi_by_campaign['silverDown'      ] = 4889 
lumi_by_campaign['silverNotGolden' ] = 4889 
#36814  B2H
#20236  BCEDF
#16578  GH
...
altCut_names.append('noEcalDriven')
...
class card_object:
    ...
    def __init__(self,
        ...
        self.PUW_names       = self.choose_fields(PUWNames      , ['NoPUW']            , selections)
...
noEcalDriven_card = card_object(name='noEcalDriven'  , selections=['noEcalDriven'],options={'kinematics':False})
...
#deck_of_cards.append(         noEcalDriven_card)
...
def make_labels(args=[], chi2=0, NDof=0, chi2OverNDof=0, vartext='', lumi=0):
    ...
    sample_text = ''
    if 'data' in args:
        sample_text = 'SingleElectron Run 2017'
        add_sample = True
...
#        labels['region'] = ROOT.TLatex(x2, y1-dy*2, '%s %s'%(region_text,e_text))
        labels['region'] = ROOT.TLatex(x2, y1-dy*3.3, '%s'%(region_text))
        labels['region'].SetTextAlign(32)
    
    altcut_texts = {
      'nominal'        : '',
      'N_1_EcalDriven'   : 'EcalDriven',
      'noEcalDriven'     : 'noEcalDriven',
