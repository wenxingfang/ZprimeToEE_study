...
parser.add_argument('--extra', '-e', help='extra input json file')
...
# Load the extra json output of combineTool.py -M Impacts
data_e = {}
with open(args.extra) as jsonfile:
    data_e = json.load(jsonfile)
...
for page in xrange(n):
    ...
    max_impact = 0.
    g_pulls_e = ROOT.TGraphAsymmErrors(n_params)
    g_impacts_hi_e = ROOT.TGraphAsymmErrors(n_params)
    g_impacts_lo_e = ROOT.TGraphAsymmErrors(n_params)
    ...
    for p in xrange(n_params):
        ...
        fit = pdata[p]['fit']
        tp = pdata[p]['type']
        para_name=pdata[p]["name"]
        ip_e=None
        for ip in data_e['params']:
            if ip["name"]==para_name: 
                ip_e=ip
        if ip_e==None: 
            print "don't find %s in extra"%(para_name)
            continue
        pre_e = ip_e['prefit']
        fit_e = ip_e['fit']
        seen_types.add(tp)
        if pdata[p]['type'] != 'Unconstrained':
            ...
            pre_err_hi = (pre_e[2] - pre_e[1])
            pre_err_lo = (pre_e[1] - pre_e[0])
            pull = fit_e[1] - pre_e[1]
            pull = (pull/pre_err_hi) if pull >= 0 else (pull/pre_err_lo)
            pull_hi = fit_e[2] - pre_e[1]
            pull_hi = (pull_hi/pre_err_hi) if pull_hi >= 0 else (pull_hi/pre_err_lo)
            pull_hi = pull_hi - pull
            pull_lo = fit_e[0] - pre_e[1]
            pull_lo = (pull_lo/pre_err_hi) if pull_lo >= 0 else (pull_lo/pre_err_lo)
            pull_lo =  pull - pull_lo
            g_pulls_e.SetPoint(i, pull, float(i) + 0.5)
            g_pulls_e.SetPointError(i, pull_lo, pull_hi, 0., 0.)
        else:
            # Hide this point
            g_pulls.SetPoint(i, 0., 9999.)
            g_pulls_e.SetPoint(i, 0., 9999.)
            y1 = ROOT.gStyle.GetPadBottomMargin()
            y2 = 1. - ROOT.gStyle.GetPadTopMargin()
        ...
        g_impacts_hi_e.SetPoint(i, 0, float(i) + 0.5)
        g_impacts_lo_e.SetPoint(i, 0, float(i) + 0.5)
        imp_e = ip_e[POIs[0]]
        g_impacts_hi_e.SetPointError(i, 0, imp_e[2] - imp_e[1], 0.5, 0.5)
        g_impacts_lo_e.SetPointError(i, imp_e[1] - imp_e[0], 0, 0.5, 0.5)
        max_impact = max(max_impact, abs(imp[1] - imp[0]), abs(imp[2] - imp[1]),abs(imp_e[1] - imp_e[0]), abs(imp_e[2] - imp_e[1]))
        col = colors.get(tp, 2)
    ...
    legend.AddEntry(g_impacts_hi_e, '+1#sigma Impact obs.', 'F')
    legend.AddEntry(g_impacts_lo_e, '-1#sigma Impact obs.', 'F')
    legend.Draw()
...
#    plot.DrawTitle(pads[1], '#hat{%s_{exp.}} = %.3g #pm %.3g%s' % (Translate(POIs[0], translate), POI_fit[1], (POI_fit[2] - POI_fit[0]) / 2., '' if args.units is None else ' '+args.units), 3)
    plot.DrawTitle_v1(pads[1], '#hat{%s}_{exp.} = %.3g #pm %.3g%s, #hat{%s}_{obs.} = %.3g #pm %.3g%s' % (Translate(POIs[0], translate), POI_fit[1], (POI_fit[2] - POI_fit[0]) / 2., '' if args.units is None else ' '+args.units, Translate(POIs[0], translate), POI_fit_e[1], (POI_fit_e[2] - POI_fit_e[0]) / 2., '' if args.units is None else ' '+args.units), 3)
    extra = ''
    if page == 0:
        extra = '('
    if page == n - 1:
