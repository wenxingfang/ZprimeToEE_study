...
//                                                                        std::cout<<"has W,from lepton and neutrion"<<",pt="<<(p4_lepton+p4_neutrion).Pt()<<std::endl;
                                                                       }
                      }
      if(isZToEE==true){
      Pt_threshold=100;
      ...
      for(unsigned int iEl=0 ; iEl<gsf_n ; ++iEl){
        ...
        if(isData){
          ...
          vector<float>* Trig_eta=0;
          vector<float>* Trig_phi=0;
          int passTrigger=0;       
          Trig_eta = trig_eta;
          Trig_phi = trig_phi;
          passTrigger = trig_fire ;

          TLorentzVector trigp4 ;
          for(unsigned int itrig=0 ; itrig<Trig_eta->size() ; ++itrig){
            trigp4.SetPtEtaPhiM(100,Trig_eta->at(itrig),Trig_phi->at(itrig),10) ;
            if(trigp4.DeltaR(el->p4) < 0.1){
              el->pass_trigger = (true && passTrigger==1) ;
              break ;
            }
          }
          }
      ...
      for(unsigned int iTag=0 ; iTag<electrons.size() ; ++iTag){
        ...
        if(tag->Et <35 || tag->accept_PreSelection==false || tag->accept_tag_ID == true || tag->region != 1 || tag->pass_trigger==0) continue ;
        }
        tag_index_out = iTag ;
        ...
        tag_passtrigger_out    = tag->pass_trigger ;
        triggerMatch = true ;
        ...
        for(unsigned int iProbe=0 ; iProbe<electrons.size() ; ++iProbe){
          ...
          probe_passtrigger_out    = probe->pass_trigger ;
