...
struct electron_candidate{
  ...
  void apply_ID_value(float value_dPhiIn, float value_Sieie, float value_missingHits, float value_dxyFirstPV, float value_HOverE, float value_scEnergy, float value_E1x5OverE5x5, float value_E2x5OverE5x5, float value_isolEMHadDepth1, float value_IsolPtTrks, float value_EcalDriven, float value_dEtaIn, float rho){
    ...
    pass_noEcalDriven = (et>35 && accept_noEcalDriven_ID && (region==1 || region==3)) ? 1 : 0 ;
...
int check_saturation(vector<int> *EBHits_rawId, int Ele_sc_seed_rawId, vector<bool> *EBHits_kSaturated, vector<int> *EEHits_rawId, vector<bool> *EEHits_kSaturated){
...
void reskim::Loop(TString fname){
   ...
   vector<int>   heeps_80_noEcalDriven              ; 
   vector<int>   heeps_80_PreSelection              ;
   vector<int>   heeps_80_match                     ; 
   ...
   vector<float> mc_electron_pt               ; 
   vector<float> mc_electron_eta              ; 
   vector<float> mc_electron_phi              ; 
   vector<float> mc_electron_m                ; 
   vector<int>   mc_electron_status           ; 
   ...
   tree_out.Branch("heeps_80_noEcalDriven"              , &heeps_80_noEcalDriven              );
   tree_out.Branch("heeps_80_PreSelection"              , &heeps_80_PreSelection              );
   tree_out.Branch("heeps_80_match"                     , &heeps_80_match                     );
   ...
   tree_out.Branch("heeps_74_saturated"                 , &heeps_74_saturated                 );
   ...
   tree_out.Branch("mc_electron_pt"               , &mc_electron_pt                    );
   tree_out.Branch("mc_electron_eta"              , &mc_electron_eta                   );
   tree_out.Branch("mc_electron_phi"              , &mc_electron_phi                   );
   tree_out.Branch("mc_electron_m"                , &mc_electron_m                     );
   tree_out.Branch("mc_electron_status"           , &mc_electron_status                );

   tree_out.Branch("LHE_Z_mass"      , &LHE_Z_mass        , "LHE_Z_mass/F"            ) ;
   tree_out.Branch("mc_Z_Pt_25"     , &mc_Z_Pt_25       , "mc_Z_Pt_25/F"           ) ;
   tree_out.Branch("mc_Z_Pt_35"     , &mc_Z_Pt_35       , "mc_Z_Pt_35/F"           ) ;
   tree_out.Branch("mc_Z_M_25"      , &mc_Z_M_25        , "mc_Z_M_25/F"            ) ;
   tree_out.Branch("mc_Z_M_35"      , &mc_Z_M_35        , "mc_Z_M_35/F"            ) ;
   ...
   bool do_EnergyCorrect_74=false;
   bool do_EnergySmear=false;
   bool save_mc_electron=true;
   if(isData==false) do_EnergyCorrect_74=false;
   else {do_EnergySmear=false;save_mc_electron=false;}
...
//// clear vector //////////////////////
      ...
      vector<int>  ().swap(heeps_80_noEcalDriven              );
      vector<int>  ().swap(heeps_80_PreSelection              );
      vector<int>  ().swap(heeps_80_match                     );
      ...
      vector<int>  ().swap(heeps_74_saturated                 );
      ...
      vector<float>().swap(mc_electron_pt               ); 
      vector<float>().swap(mc_electron_eta              ); 
      vector<float>().swap(mc_electron_phi              ); 
      vector<float>().swap(mc_electron_m                ); 
      vector<int>  ().swap(mc_electron_status           ); 
...
//   if(LHE_weight_nominal!=-1)   std::cout<<"LHE size"<<LHE_id_sys->size()<<std::endl;
...
/*
      ...
      if(isData==false){
      TLorentzVector MC_p4_Z_1(0,0,0,0);
      TLorentzVector MC_p4_Z_2(0,0,0,0);
      TLorentzVector tmp_p4(0,0,0,0);
      int N_ele_25 = 0 ;
      int N_ele_35 = 0 ;
      for(unsigned iMC=0 ; iMC<mc_n ; ++iMC){
      if(abs(mc_pdgId->at(iMC))== 11 && mc_status->at(iMC)==23 && mc_pt->at(iMC)>25){N_ele_25++;tmp_p4.SetPtEtaPhiM(mc_pt->at(iMC),mc_eta->at(iMC),mc_phi->at(iMC),mc_mass->at(iMC));MC_p4_Z_1=MC_p4_Z_1+tmp_p4;}
      if(abs(mc_pdgId->at(iMC))== 11 && mc_status->at(iMC)==23 && mc_pt->at(iMC)>35){N_ele_35++;tmp_p4.SetPtEtaPhiM(mc_pt->at(iMC),mc_eta->at(iMC),mc_phi->at(iMC),mc_mass->at(iMC));MC_p4_Z_2=MC_p4_Z_2+tmp_p4;}
      }
      if(N_ele_25==2){mc_Z_Pt_25=MC_p4_Z_1.Pt();mc_Z_M_25=MC_p4_Z_1.M();}
      if(N_ele_35==2){mc_Z_Pt_35=MC_p4_Z_2.Pt();mc_Z_M_35=MC_p4_Z_2.M();}
      }
*/
      LHE_weight=1;
      LHE_Z_mass=1;
      ...
      if( abs(LHE_pdgid->at(iL))==6) {tmp_weight=tmp_weight*(exp(-1.08872e+00-(1.19998e-02)*LHE_Pt->at(iL))+8.95139e-01);LHE_top_pt.push_back(LHE_Pt->at(iL));}
      }
...
//      pv_totTrackSize_out  .push_back(pv_totTrackSize  ->at(ip)) ;
      ...
      if(save_mc_electron){
      for(unsigned iMC=0 ; iMC<mc_n ; ++iMC){
      if(abs(mc_pdgId->at(iMC)) != 11 ) continue;
      mc_electron_pt    .push_back(mc_pt->at(iMC));
      mc_electron_eta   .push_back(mc_eta->at(iMC));
      mc_electron_phi   .push_back(mc_phi->at(iMC));
      mc_electron_m     .push_back(mc_mass->at(iMC));
      mc_electron_status.push_back(mc_status->at(iMC));
      }
      }
//#############   For electrons ##########################
...
///////////////////For Z' to ee study /////////////////////
        ...
        heep_80->truthmatched =gsf_mc_match;
        ...
        heep_74->saturated    =check_saturation(EBHits_rawId, Ele_sc_seed_rawId, EBHits_kSaturated, EEHits_rawId, EEHits_kSaturated);
...
//      std::cout<<"LHE size"<<LHE_id_sys->size()<<std::endl;
      for(unsigned int j=0; j<LHE_id_sys->size(); j++){
      int tmp_id=atoi(LHE_id_sys->at(j).c_str());
      if((tmp_id>=1001 && tmp_id<=1009)||(tmp_id>=2001 && tmp_id<=2102) ||(tmp_id<=110)){
      LHE_sys_weight.push_back(LHE_weight_sys->at(j)/LHE_weight_nominal);
      LHE_sys_id    .push_back(tmp_id);
//      if(tmp_id==5 || tmp_id==1005 || tmp_id==9 || tmp_id==1009)std::cout<<"s id="<<LHE_id_sys->at(j)<<", int id="<<tmp_id<<",weight="<<LHE_weight_sys->at(j)/LHE_weight_nominal<<std::endl;
                                      }
      }
      }
      ...
      heeps_80_noEcalDriven.push_back(heeps_80.at(j)->pass_noEcalDriven); 
      heeps_80_PreSelection.push_back(heeps_80.at(j)->pass_PreSelection); 
      heeps_80_match       .push_back(heeps_80.at(j)->truthmatched     ); 
      }
      ...
      heeps_74_saturated      .push_back(heeps_74.at(j)->saturated          ); 
      }
