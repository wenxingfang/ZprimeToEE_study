...
def draw_plots(date, out_dir, h_stack,h_mc,g_mc,h_data,out_name, blind):
    ...
    pad1 = ROOT.TPad('pad1', '',0,0.23, 1,1)
    pad2 = ROOT.TPad('pad2', '',0,0,1.0,0.2622)
    pad2.Draw()
    pad1.Draw()
    pad1.cd()
    pad1.Range(1.488995,-5.792772,3.713274,10.06267)
    pad1.SetFillColor(0)
    ...
    pad2.cd()
    pad2.Range(1.488995,-2.220339,3.713274,1.169492)
    pad2.SetFillColor(0)
...
#    g_data.Draw("pZ0")
    ...
    if out_name in ['h_mee_all','h_mee_all_BB','h_mee_all_BE']:
        shrunk_x=0.09
        legend_x1=0.536741-shrunk_x
        legend_x2=0.815536-shrunk_x
        legend_text_size=0.055
    legend = ROOT.TLegend(legend_x1,0.57,legend_x2,0.88,"","brNDC");
#    legend.AddEntry(g_tmp_data,'Data','ep')
#    legend.AddEntry(g_data,'Data','ep')
    legend.AddEntry(g_data_clone,'Data','ep')
    ...
    #label_cms.Draw()
    label_CMS= ROOT.TLatex(0.13,0.92,"CMS")
    label_CMS.SetNDC()
    label_CMS.SetTextSize(0.075)
    label_CMS.SetLineWidth(2)
    label_CMS.Draw()
    ...
    #label_prelim.Draw()
    label_Prelim =ROOT.TLatex(0.25705,0.92,"Preliminary")
    label_Prelim.SetNDC()
    label_Prelim.SetTextFont(52)
    label_Prelim.SetTextSize(0.06)
    label_Prelim.SetLineWidth(2)
    label_Prelim.Draw()
    ...
    if isSupplementStyle:
        label_supply.Draw()
    label_lumi = ROOT.TPaveLabel(0.74, 0.88, 0.99, 0.96, "137 fb^{-1} (13 TeV)", "brNDC")
    #label_lumi = ROOT.TPaveLabel(0.74, 0.88, 0.99, 0.96, "%.1f fb^{-1} (13 TeV)"%float((35922.0+41500.0+59401.0)/1000), "brNDC")
    ...
    #label_lumi.Draw()
    #label_Lumi = ROOT.TLatex(0.95,0.92,"%.1f fb^{-1} (13 TeV)"%float((35922.0+41500.0+59401.0)/1000))
    label_Lumi = ROOT.TLatex(0.95,0.92,"137 fb^{-1} (13 TeV)")
    label_Lumi.SetNDC()
    label_Lumi.SetTextAlign(31)
    label_Lumi.SetTextFont(42)
    label_Lumi.SetTextSize(0.06)
    label_Lumi.SetLineWidth(2)
    label_Lumi.Draw()
    ...
    dummy_ratio.GetXaxis().SetTitleSize(0.1762014)
    dummy_ratio.GetXaxis().SetLabelSize(0.1292143)
    ...
    dummy_ratio.GetYaxis().SetTitleSize(0.1204043)
    dummy_ratio.GetYaxis().SetLabelSize(0.1550572)
    ...
    dummy_ratio.GetXaxis().SetLabelFont(42)
    dummy_ratio.GetYaxis().SetLabelFont(42)
    dummy_ratio.Draw()
...
#    canvas.SaveAs('%s/%s/%s.root'%(out_dir,date,out_name))    
    ...
    if out_name in ["h_mee_all",'h_mee_all_cumlative_v1'] and True:
        h_ZToEE_mee=0;
        h_ttbar_other_mee=0;
        h_jets_mee=0;
        ...
        print h_jets_mee
        ####### for combine result ###################
        f_comb= ROOT.TFile("Result_Run2_PAS.root","update")
        f_comb.cd()
        ...
        f_HEPData= ROOT.TFile("HEPData_Run2_paper.root","update")
        f_HEPData.cd()
        ...
        g_ratio      .Write("ratio_%s"%out_name  ,ROOT.TObject.kOverwrite)
        g_mc_stat_sys.Write("uncert_%s"%out_name ,ROOT.TObject.kOverwrite)
        f_HEPData.Close()
#    ################ Fit background ####################
...
def Set_and_Draw(date, out_dir, hname, System_uncertainty, total_hist, ttbar_other_hist, ZToEE_hist, Jets_hist, ASF_dict):
    ...
    for hist in System_uncertainty["nominal"].sample_hist["data"]:
        ...
        if Fix_BB_Jet and hname=='h_mee_all_BB':
            tmp_x_axis=h_Jets.GetXaxis()
            fix_bin=tmp_x_axis.FindBin(1025)
            h_Jets.SetBinContent(fix_bin,(h_Jets.GetBinContent(fix_bin-1)+h_Jets.GetBinContent(fix_bin+1))/2)
        stack_mc.Add(h_Jets)
...
class file_object:
    def __init__(self, file_name, name, event, crosssection, color):
        ...
        #self.tfilename = 'ntuples/sys_saved_hist/Run2/%s.root'%(self.file_name)
        #self.tfilename = 'ntuples/sys_saved_hist/Run2_EENoTTW/%s.root'%(self.file_name)
        #self.tfilename = 'ntuples/sys_saved_hist/Run2_final/%s.root'%(self.file_name)
        #self.tfilename = 'ntuples/sys_saved_hist/Run2_final_0704/%s.root'%(self.file_name)
        self.tfilename = 'ntuples/sys_saved_hist/Run2_final_0810/%s.root'%(self.file_name)
...
class collcet_hist:
    def __init__(self, name, fewz_correct, TTbar_MLL, WW_MLL, sys_uncert):
        ...
        elif self.sys_uncert=="normalization_scale_up"    : self.str_uncert="normalization_scale_up_"
        elif self.sys_uncert=="normalization_scale_down"  : self.str_uncert="normalization_scale_down_"
        elif self.sys_uncert=="BB_mass_scale_up_2016"     : self.str_uncert="BB_mass_scale_up_2016_"
        elif self.sys_uncert=="BB_mass_scale_down_2016"   : self.str_uncert="BB_mass_scale_down_2016_"
        elif self.sys_uncert=="BE_mass_scale_up_2016"     : self.str_uncert="BE_mass_scale_up_2016_"
        elif self.sys_uncert=="BE_mass_scale_down_2016"   : self.str_uncert="BE_mass_scale_down_2016_"
        elif self.sys_uncert=="BB_mass_scale_up_2017"     : self.str_uncert="BB_mass_scale_up_2017_"
        elif self.sys_uncert=="BB_mass_scale_down_2017"   : self.str_uncert="BB_mass_scale_down_2017_"
        elif self.sys_uncert=="BE_mass_scale_up_2017"     : self.str_uncert="BE_mass_scale_up_2017_"
        elif self.sys_uncert=="BE_mass_scale_down_2017"   : self.str_uncert="BE_mass_scale_down_2017_"
        elif self.sys_uncert=="BB_mass_scale_up_2018"     : self.str_uncert="BB_mass_scale_up_2018_"
        elif self.sys_uncert=="BB_mass_scale_down_2018"   : self.str_uncert="BB_mass_scale_down_2018_"
        elif self.sys_uncert=="BE_mass_scale_up_2018"     : self.str_uncert="BE_mass_scale_up_2018_"
        elif self.sys_uncert=="BE_mass_scale_down_2018"   : self.str_uncert="BE_mass_scale_down_2018_"
...
#        self.DY_pdf_6000_Inf =DY_pdf_new(6000,7500)
        ...
        self.Xsection={}
        self.Xsection['ZToEE_50_120']   =  1
        self.Xsection['ZToEE_120_200']  =  1 
        self.Xsection['ZToEE_200_400']  =  1 
        self.Xsection['ZToEE_400_800']  =  1 
        self.Xsection['ZToEE_800_1400'] =  1  
        self.Xsection['ZToEE_1400_2300']=  1   
        self.Xsection['ZToEE_2300_3500']=  1     
        self.Xsection['ZToEE_3500_4500']=  1  
        self.Xsection['ZToEE_4500_6000']=  1  
        self.Xsection['ZToEE_6000_Inf'] =  1  

        self.Xsection['NNPDF31_ZToEE_50_120']   = 1
        self.Xsection['NNPDF31_ZToEE_120_200']  = 1
        self.Xsection['NNPDF31_ZToEE_200_400']  = 1
        self.Xsection['NNPDF31_ZToEE_400_800']  = 1
        self.Xsection['NNPDF31_ZToEE_800_1400'] = 1
        self.Xsection['NNPDF31_ZToEE_1400_2300']= 1
        self.Xsection['NNPDF31_ZToEE_2300_3500']= 1
        self.Xsection['NNPDF31_ZToEE_3500_4500']= 1
        self.Xsection['NNPDF31_ZToEE_4500_6000']= 1
        self.Xsection['NNPDF31_ZToEE_6000_Inf'] = 1

        self.Xsection['ZToTT']          =  1
        self.Xsection['TTbar']          =  1
        self.Xsection['TTbar_500_800']  =  1
        self.Xsection['TTbar_800_1200'] =  1
        self.Xsection['TTbar_1200_1800']=  1
        self.Xsection['TTbar_1800_Inf'] =  1
        self.Xsection['WW_inc']         =  1
        self.Xsection['WW']             =  1
        self.Xsection['WW_200_600']     =  1
        self.Xsection['WW_600_1200']    =  1
        self.Xsection['WW_1200_2500']   =  1
        self.Xsection['WW_2500_Inf']    =  1
        self.Xsection['WZ_inc']         =  1
        self.Xsection['WZ_3LNu']        =  1
        self.Xsection['WZ_2L2Q']        =  1
        self.Xsection['ZZ_inc']         =  1
        self.Xsection['ZZ_2L2Q']        =  1
        self.Xsection['ZZ_2L2Nu']       =  1
        self.Xsection['ZZ_4L']          =  1
        self.Xsection['ST']             =  1
        self.Xsection['ST_anti']        =  1
        ...
        if self.sys_uncert == "bgk_ZToTT_scale_up":
            self.Xsection['ZToTT']      =  (1+self.Xsection_uncer["DY"])*self.Xsection['ZToTT']      
    ...
    def get_input_file(self):
        ...
        self.files['ZToEE_50_120']       =file_object(self.str_uncert_Z+'hist_ZToEE_50_120','Z#rightarrowee'     , 1,    1,    ROOT.kGreen)
        self.files['ZToEE_120_200']      =file_object(self.str_uncert_Z+'hist_ZToEE_120_200','ZToEE_120_200'     , 1,    1,    ROOT.kGreen)
        self.files['ZToEE_200_400']      =file_object(self.str_uncert_Z+'hist_ZToEE_200_400','ZToEE_200_400'     , 1,    1,    ROOT.kGreen)
        self.files['ZToEE_400_800']      =file_object(self.str_uncert_Z+'hist_ZToEE_400_800','ZToEE_400_800'     , 1,    1,    ROOT.kGreen)
        self.files['ZToEE_800_1400']     =file_object(self.str_uncert_Z+'hist_ZToEE_800_1400','ZToEE_800_1400'   , 1,    1,    ROOT.kGreen)
        self.files['ZToEE_1400_2300']    =file_object(self.str_uncert_Z+'hist_ZToEE_1400_2300','ZToEE_1400_2300' , 1,    1,    ROOT.kGreen)
        self.files['ZToEE_2300_3500']    =file_object(self.str_uncert_Z+'hist_ZToEE_2300_3500','ZToEE_2300_3500' , 1,    1,    ROOT.kGreen)
        self.files['ZToEE_3500_4500']    =file_object(self.str_uncert_Z+'hist_ZToEE_3500_4500','ZToEE_3500_4500' , 1,    1,    ROOT.kGreen)
        self.files['ZToEE_4500_6000']    =file_object(self.str_uncert_Z+'hist_ZToEE_4500_6000','ZToEE_4500_6000' , 1,    1,    ROOT.kGreen)
        self.files['ZToEE_6000_Inf']     =file_object(self.str_uncert_Z+'hist_ZToEE_6000_Inf','ZToEE_6000_Inf'   , 1,    1,    ROOT.kGreen)
        self.files['ZToTT']              =file_object(self.str_uncert  +'hist_ZToTT','Z#rightarrow#tau#tau'      , 1,    self.Xsection['ZToTT']          ,    ROOT.kCyan  )
        self.files['TTbar']              =file_object(self.str_uncert  +'hist_TTbar','TTbar'                     , 1,    self.Xsection['TTbar']          ,    ROOT.kYellow)
        self.files['TTbar_500_800']      =file_object(self.str_uncert  +'hist_TTbar_500_800','TTbar_500_800'     , 1,    self.Xsection['TTbar_500_800']  ,    ROOT.kYellow)
        self.files['TTbar_800_1200']     =file_object(self.str_uncert  +'hist_TTbar_800_1200','TTbar_800_1200'   , 1,    self.Xsection['TTbar_800_1200'] ,    ROOT.kYellow)
        self.files['TTbar_1200_1800']    =file_object(self.str_uncert  +'hist_TTbar_1200_1800','TTbar_1200_1800' , 1,    self.Xsection['TTbar_1200_1800'],    ROOT.kYellow)
        self.files['TTbar_1800_Inf']     =file_object(self.str_uncert  +'hist_TTbar_1800_Inf','TTbar_1800_Inf'   , 1,    self.Xsection['TTbar_1800_Inf'] ,    ROOT.kYellow)
        self.files['ST']                 =file_object(self.str_uncert  +'hist_ST','Single Top'                   , 1,    self.Xsection['ST']             ,    ROOT.kRed)
        self.files['ST_anti']            =file_object(self.str_uncert  +'hist_ST_anti','ST_anti'                 , 1,    self.Xsection['ST_anti']        ,    ROOT.kRed)
        self.files['WW']                 =file_object(self.str_uncert  +'hist_WW','Di-Boson'                     , 1,    self.Xsection['WW']             ,    ROOT.kMagenta)
        self.files['WW_200_600']         =file_object(self.str_uncert  +'hist_WW_200_600','WW2L_200_600'         , 1,    self.Xsection['WW_200_600']     ,    ROOT.kMagenta)
        self.files['WW_600_1200']        =file_object(self.str_uncert  +'hist_WW_600_1200','WW2L_600_1200'       , 1,    self.Xsection['WW_600_1200']    ,    ROOT.kMagenta)
        self.files['WW_1200_2500']       =file_object(self.str_uncert  +'hist_WW_1200_2500','WW2L_1200_2500'     , 1,    self.Xsection['WW_1200_2500']   ,    ROOT.kMagenta)
        self.files['WW_2500_Inf']        =file_object(self.str_uncert  +'hist_WW_2500_Inf','WW2L_2500_Inf'       , 1,    self.Xsection['WW_2500_Inf']    ,    ROOT.kMagenta)
        self.files['WZ_2L2Q']            =file_object(self.str_uncert  +'hist_WZ_2L2Q','WZ_2L2Q'                 , 1,    self.Xsection['WZ_2L2Q']        ,    ROOT.kMagenta)
        self.files['WZ_3LNu']            =file_object(self.str_uncert  +'hist_WZ_3LNu','WZ_3LNu'                 , 1,    self.Xsection['WZ_3LNu']        ,    ROOT.kMagenta)
        self.files['ZZ_2L2Nu']           =file_object(self.str_uncert  +'hist_ZZ_2L2Nu','ZZ_2L2Nu'               , 1,    self.Xsection['ZZ_2L2Nu']       ,    ROOT.kMagenta)
        self.files['ZZ_2L2Q']            =file_object(self.str_uncert  +'hist_ZZ_2L2Q' ,'ZZ_2L2Q'                , 1,    self.Xsection['ZZ_2L2Q']        ,    ROOT.kMagenta)
        self.files['ZZ_4L']              =file_object(self.str_uncert  +'hist_ZZ_4L'   ,'ZZ_4L'                  , 1,    self.Xsection['ZZ_4L']          ,    ROOT.kMagenta)

        self.files_1F['data']               =file_object('hist_data_FR1F_MiniAOD','data'     ,self.data_lumi      ,1     ,  ROOT.kBlack)
        self.files_1F['ZToEE_50_120']       =file_object(self.str_uncert_Z+'hist_ZToEE_50_120_1F','Z#rightarrowee'     , 1,    1,    ROOT.kGreen)
        self.files_1F['ZToEE_120_200']      =file_object(self.str_uncert_Z+'hist_ZToEE_120_200_1F','ZToEE_120_200'     , 1,    1,    ROOT.kGreen)
        self.files_1F['ZToEE_200_400']      =file_object(self.str_uncert_Z+'hist_ZToEE_200_400_1F','ZToEE_200_400'     , 1,    1,    ROOT.kGreen)
        self.files_1F['ZToEE_400_800']      =file_object(self.str_uncert_Z+'hist_ZToEE_400_800_1F','ZToEE_400_800'     , 1,    1,    ROOT.kGreen)
        self.files_1F['ZToEE_800_1400']     =file_object(self.str_uncert_Z+'hist_ZToEE_800_1400_1F','ZToEE_800_1400'   , 1,    1,    ROOT.kGreen)
        self.files_1F['ZToEE_1400_2300']    =file_object(self.str_uncert_Z+'hist_ZToEE_1400_2300_1F','ZToEE_1400_2300' , 1,    1,    ROOT.kGreen)
        self.files_1F['ZToEE_2300_3500']    =file_object(self.str_uncert_Z+'hist_ZToEE_2300_3500_1F','ZToEE_2300_3500' , 1,    1,    ROOT.kGreen)
        self.files_1F['ZToEE_3500_4500']    =file_object(self.str_uncert_Z+'hist_ZToEE_3500_4500_1F','ZToEE_3500_4500' , 1,    1,    ROOT.kGreen)
        self.files_1F['ZToEE_4500_6000']    =file_object(self.str_uncert_Z+'hist_ZToEE_4500_6000_1F','ZToEE_4500_6000' , 1,    1,    ROOT.kGreen)
        self.files_1F['ZToEE_6000_Inf']     =file_object(self.str_uncert_Z+'hist_ZToEE_6000_Inf_1F','ZToEE_6000_Inf'   , 1,    1,    ROOT.kGreen)
        self.files_1F['ZToTT']              =file_object(self.str_uncert  +'hist_ZToTT_1F','Z#rightarrow#tau#tau'      , 1,    self.Xsection['ZToTT']          ,    ROOT.kCyan  )
        self.files_1F['TTbar']              =file_object(self.str_uncert  +'hist_TTbar_1F','TTbar'                     , 1,    self.Xsection['TTbar']          ,    ROOT.kYellow)
        self.files_1F['TTbar_500_800']      =file_object(self.str_uncert  +'hist_TTbar_500_800_1F','TTbar_500_800'     , 1,    self.Xsection['TTbar_500_800']  ,    ROOT.kYellow)
        self.files_1F['TTbar_800_1200']     =file_object(self.str_uncert  +'hist_TTbar_800_1200_1F','TTbar_800_1200'   , 1,    self.Xsection['TTbar_800_1200'] ,    ROOT.kYellow)
        self.files_1F['TTbar_1200_1800']    =file_object(self.str_uncert  +'hist_TTbar_1200_1800_1F','TTbar_1200_1800' , 1,    self.Xsection['TTbar_1200_1800'],    ROOT.kYellow)
        self.files_1F['TTbar_1800_Inf']     =file_object(self.str_uncert  +'hist_TTbar_1800_Inf_1F','TTbar_1800_Inf'   , 1,    self.Xsection['TTbar_1800_Inf'] ,    ROOT.kYellow)
        self.files_1F['ST']                 =file_object(self.str_uncert  +'hist_ST_1F','Single Top'                   , 1,    self.Xsection['ST']             ,    ROOT.kRed)
        self.files_1F['ST_anti']            =file_object(self.str_uncert  +'hist_ST_anti_1F','ST_anti'                 , 1,    self.Xsection['ST_anti']        ,    ROOT.kRed)
        self.files_1F['WW']                 =file_object(self.str_uncert  +'hist_WW_1F','Di-Boson'                     , 1,    self.Xsection['WW']             ,    ROOT.kMagenta)
        self.files_1F['WW_200_600']         =file_object(self.str_uncert  +'hist_WW_200_600_1F','WW2L_200_600'         , 1,    self.Xsection['WW_200_600']     ,    ROOT.kMagenta)
        self.files_1F['WW_600_1200']        =file_object(self.str_uncert  +'hist_WW_600_1200_1F','WW2L_600_1200'       , 1,    self.Xsection['WW_600_1200']    ,    ROOT.kMagenta)
        self.files_1F['WW_1200_2500']       =file_object(self.str_uncert  +'hist_WW_1200_2500_1F','WW2L_1200_2500'     , 1,    self.Xsection['WW_1200_2500']   ,    ROOT.kMagenta)
        self.files_1F['WW_2500_Inf']        =file_object(self.str_uncert  +'hist_WW_2500_Inf_1F','WW2L_2500_Inf'       , 1,    self.Xsection['WW_2500_Inf']    ,    ROOT.kMagenta)
        self.files_1F['WZ_2L2Q']            =file_object(self.str_uncert  +'hist_WZ_2L2Q_1F','WZ_2L2Q'                 , 1,    self.Xsection['WZ_2L2Q']        ,    ROOT.kMagenta)
        self.files_1F['WZ_3LNu']            =file_object(self.str_uncert  +'hist_WZ_3LNu_1F','WZ_3LNu'                 , 1,    self.Xsection['WZ_3LNu']        ,    ROOT.kMagenta)
        self.files_1F['ZZ_2L2Nu']           =file_object(self.str_uncert  +'hist_ZZ_2L2Nu_1F','ZZ_2L2Nu'               , 1,    self.Xsection['ZZ_2L2Nu']       ,    ROOT.kMagenta)
        self.files_1F['ZZ_2L2Q']            =file_object(self.str_uncert  +'hist_ZZ_2L2Q_1F' ,'ZZ_2L2Q'                , 1,    self.Xsection['ZZ_2L2Q']        ,    ROOT.kMagenta)
        self.files_1F['ZZ_4L']              =file_object(self.str_uncert  +'hist_ZZ_4L_1F'   ,'ZZ_4L'                  , 1,    self.Xsection['ZZ_4L']          ,    ROOT.kMagenta)
    ...
    def get_jets_hist(self):
        ...
        #f_jets=ROOT.TFile("ntuples/sys_saved_hist/Run2/"+self.str_uncert+"hist_jets.root","RECREATE")
        #f_jets=ROOT.TFile("ntuples/sys_saved_hist/Run2_EENoTTW/"+self.str_uncert+"hist_jets.root","RECREATE")
        #f_jets=ROOT.TFile("ntuples/sys_saved_hist/Run2_final/"+self.str_uncert+"hist_jets.root","RECREATE")
        #f_jets=ROOT.TFile("ntuples/sys_saved_hist/Run2_final_0704/"+self.str_uncert+"hist_jets.root","RECREATE")
        f_jets=ROOT.TFile("ntuples/sys_saved_hist/Run2_final_0810/"+self.str_uncert+"hist_jets.root","RECREATE")
...
System_list.append("BB_mass_scale_up_2016"  )
System_list.append("BB_mass_scale_down_2016")
System_list.append("BE_mass_scale_up_2016"  )
System_list.append("BE_mass_scale_down_2016")
System_list.append("BB_mass_scale_up_2017"  )
System_list.append("BB_mass_scale_down_2017")
System_list.append("BE_mass_scale_up_2017"  )
System_list.append("BE_mass_scale_down_2017")
System_list.append("BB_mass_scale_up_2018"  )
System_list.append("BB_mass_scale_down_2018")
System_list.append("BE_mass_scale_up_2018"  )
System_list.append("BE_mass_scale_down_2018")
