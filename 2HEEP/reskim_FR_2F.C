#define reskim_FR_2F_cxx
#include "reskim_FR_2F.h"
#include <TH2.h>
#include <TAxis.h>
#include <TStyle.h>
#include <TCanvas.h>
...
void reskim_FR_2F::Loop(TString fname){
   if (fChain == 0) return;
   ...
   bool do_L1_seed=false;
   bool do_fakeRate_fail_fail=true;
...
//####################################################          
     ...
     if(do_fakeRate_fail_fail==true){ if(e1->Et<35 || true==e1->accept_HEEP_ID || false==e1->pass_trigger || e1->accept_PreSelection==false) continue ;}
        
      for(unsigned int i_2=i_1+1 ; i_2<electrons.size() ; ++i_2){
      electron_candidate* e2 = electrons.at(i_2) ;
      if(do_fakeRate_fail_fail==true){if(e2->Et<35 || true==e2->accept_HEEP_ID || false==e2->pass_trigger || e2->accept_PreSelection==false) continue ;}
